...
<div>
	...
	<div>
		...
		<c:forEach items="${jobNameList}" var="jobName">
			<option value="${jobName}" <c:if test="${data.jobName == jobName}">selected</c:if>>${jobName}</option>
		</c:forEach>
		</select>
		</label>
		...
		<input id="jobDescription" name="jobDescription" value="${data.jobDescription }" type="hidden"/>
		</label>
...
<c:if test="${l.triggers.size() == 0}">
	<tr>
		...
		|<a href="deleteJob?jobGroup=${job.group}&jobName=${job.name}" title="删除job">删1除</a>
		</td>
	</tr>
</c:if>
<c:if test="${l.triggers.size() != 0}">
	...
	<tr>
		...
		<c:if test="${trigger.state == '已删除'}">已删除</c:if>
		|<a href="unscheduleJob?triggerGroup=${trigger.group}&triggerName=${trigger.name}" title="解除trigger">解除</a>
		|<a href="deleteJob?jobGroup=${l.group}&jobName=${l.name}" title="删除job">删除</a>
		</td>
	</tr>
	</c:forEach>
</c:if>
